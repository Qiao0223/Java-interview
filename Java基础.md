...
# 32. G1（Garbage First）垃圾回收器的回收步骤
...
进程（Process）和线程（Thread）是操作系统中的两个重要概念。**进程是资源分配的最小单位，线程是 CPU 调度的最小单位**。
...
|---|---|---|
|**基本定义**|**独立运行的程序**，拥有独立的资源|**进程内部的执行单元**，共享进程资源|
|**调度单位**|**资源分配的最小单位**|**CPU 调度的最小单位**|
|**资源**|**独立地址空间**，每个进程都有自己的内存、文件、变量等|**共享进程的资源**（内存、文件句柄、全局变量等）|
|**通信方式**|进程间通信（IPC），如 **管道、消息队列、共享内存、Socket**|线程间直接共享数据，**通信开销低**|
|**切换开销**|进程切换（上下文切换）**开销大**，涉及**完整的资源切换**|线程切换**开销小**，仅需切换寄存器和堆栈|
|**独立性**|**进程间相互独立**，一个进程崩溃不会影响其他进程|**线程间影响较大**，一个线程崩溃可能影响整个进程|
|**并发性**|**多进程并发，适合 CPU 密集型任务**|**多线程并发，适合 I/O 密集型任务**|
|**创建方式**|`fork()`（Linux）、`CreateProcess()`（Windows）|`pthread_create()`（Linux）、`CreateThread()`（Windows）|
|**适用场景**|**浏览器、多任务 OS、微服务架构**|**Web 服务器、Java 线程池、高并发任务**|
...
- 打开 **多个 Chrome 浏览器窗口**，每个窗口是一个**独立的进程**，彼此独立运行，**某个窗口崩溃不会影响其他窗口**。

### **✅ 2. 线程**

- 在一个 **Chrome 进程内部**，打开多个 **标签页**，每个标签页可能是**一个线程**，但它们共享相同的内存和资源。
...
|**场景**|**使用进程**|**使用线程**|
|---|---|---|
|**Web 服务器（Nginx、Tomcat）**|❌|✅（多线程处理请求）|
|**数据库（MySQL、PostgreSQL）**|✅（多进程架构）|✅（线程池并发查询）|
|**视频播放器**|✅（音频解码、视频解码分进程）|✅（播放、缓存、字幕加载多线程）|
|**分布式微服务**|✅（不同服务进程独立运行）|❌|
|**游戏引擎**|❌|✅（物理引擎、渲染引擎多线程）|
...
1. **进程是独立运行的程序**，有自己**独立的资源（内存、文件等）**，不同进程**互不影响**。
2. **线程是进程内部的执行单元**，多个线程**共享同一个进程的资源**，线程切换**比进程更快**。
3. **多进程适用于高隔离、高可靠性场景**（如数据库、浏览器）。
4. **多线程适用于高并发、共享资源的场景**（如 Web 服务器、线程池）。
...
# 50. Java 四种引用类型（强引用、软引用、弱引用、虚引用）
...
# 51. String vs. StringBuffer vs. StringBuilder 对比
...
# 56. ArrayList vs LinkedList：海量数据写入 & 删除的选择
...
# 58. CHW（Consistent Hashing with Virtual Nodes）实现原理
...
Java **IO（Input/Output）流** 是 **用于处理数据读写的 API**，支持 **文件、网络、内存等数据源** 的 **输入/输出操作**。
...
|**类型**|**作用**|**常见类**|
|---|---|---|
|**输入流（InputStream / Reader）**|**从外部读取数据**|`FileInputStream`、`BufferedReader`|
|**输出流（OutputStream / Writer）**|**向外部写入数据**|`FileOutputStream`、`BufferedWriter`|

### **📌 2. 按照数据类型分类**

|**类型**|**作用**|**常见类**|
|---|---|---|
|**字节流（InputStream / OutputStream）**|**处理二进制数据（图片、音频、视频）**|`FileInputStream`、`ByteArrayOutputStream`|
|**字符流（Reader / Writer）**|**处理文本数据（字符、字符串）**|`FileReader`、`BufferedWriter`|

### **📌 3. 按照流的功能分类**

|**类型**|**作用**|**常见类**|
|---|---|---|
|**节点流（基本流）**|**直接操作文件/数据源**|`FileInputStream`、`FileReader`|
|**处理流（包装流）**|**提高性能、增加功能**|`BufferedReader`、`ObjectInputStream`|
...
[字节流] ────────┤
                 └─── OutputStream (字节输出流) ─── FileOutputStream, BufferedOutputStream
...
|**流**|**作用**|**常见方法**|
|---|---|---|
|**FileInputStream / FileOutputStream**|**读写文件（二进制数据）**|`read()`、`write()`|
|**BufferedReader / BufferedWriter**|**缓冲字符，提高性能**|`readLine()`、`flush()`|
|**ObjectInputStream / ObjectOutputStream**|**序列化对象**|`readObject()`、`writeObject()`|
...
- **传统 IO（BIO）**：同步阻塞（`InputStream/OutputStream`）。
- **新 IO（NIO）**：非阻塞、基于 **Channel + Buffer** 进行数据处理，提高性能。
- **适用于高并发（如 Netty、Web 服务器）**。
...
|**分类**|**输入流**|**输出流**|**作用**|
|---|---|---|---|
|**字节流**|`InputStream`|`OutputStream`|处理 **二进制数据**|
|**字符流**|`Reader`|`Writer`|处理 **文本数据**|
|**缓冲流**|`BufferedReader`|`BufferedWriter`|**提高 IO 速度**|
|**对象流**|`ObjectInputStream`|`ObjectOutputStream`|**对象序列化**|

# 60. 双亲委派机制

双亲委派机制是 Java 类加载器（ClassLoader）的一种工作模式，其核心思想是**让类加载请求逐级向上委托，只有在上层找不到对应的类时，才由当前 ClassLoader 加载**。这样可以保证 Java 类的安全性和稳定性，避免重复加载同一个类。

---

## **1. 双亲委派机制的原理**

双亲委派机制采用层次化结构，由上至下逐级委派，类加载顺序如下：

1. **Bootstrap ClassLoader（启动类加载器）**
    - 负责加载 Java 核心类库，如 `rt.jar`、`java.lang.*` 等。
2. **Extension ClassLoader（扩展类加载器）**
    - 负责加载 `jre/lib/ext/` 目录或 `java.ext.dirs` 指定的扩展类。
3. **Application ClassLoader（应用类加载器）**
    - 负责加载 `classpath` 下的类（即用户编写的 Java 类）。
4. **自定义 ClassLoader（用户自定义类加载器）**
    - 由用户编写，通常继承 `ClassLoader` 进行自定义类加载行为。

### **委派流程**

当一个类需要被加载时：

1. 先由当前 ClassLoader 发送加载请求；
2. 委托给父加载器（即上一级 ClassLoader）；
3. 依次向上递归，最终交给 `Bootstrap ClassLoader`；
4. 如果父级加载器无法加载，则由当前 ClassLoader 自行加载。
...
1. **避免类的重复加载**
    - 确保同一个类在 JVM 中只被加载一次，避免不同 ClassLoader 重新加载相同类导致的冲突。
2. **保证 Java 的核心类库安全**
    - 防止开发者自定义 `java.lang.String` 等核心类，避免类被篡改或破坏。
3. **提高性能**
    - 由于类一般在父级加载器中已经加载，避免重复查找和加载，提高系统效率。

---

## **3. 破坏双亲委派机制**

虽然双亲委派机制在大多数情况下有效，但在某些场景下可能需要打破这种机制，例如：

1. **Tomcat、OSGi、JDBC Driver、SPI（服务提供接口）**
    - 需要动态加载类，允许不同模块加载相同的类但版本不同。
2. **用户自定义类加载器**
    - 通过继承 `ClassLoader` 并重写 `loadClass()` 方法，自定义类的加载逻辑，如 `findClass()`。

## **4. 结论**

- **双亲委派机制** 主要用于 Java 类加载，保证类的安全性和稳定性。
- **在某些情况下（如插件加载），可以打破双亲委派**，但需要小心处理，避免类冲突和安全隐患。
- **理解类加载机制** 对于编写高效、安全的 Java 代码至关重要，尤其是涉及自定义 ClassLoader 的场景。

# 61. 多线程任务，一个线程在等待的时候会占用cpu资源吗？

### **多线程任务：线程在等待时会占用 CPU 资源吗？**

**不一定**，线程是否占用 **CPU 资源**，取决于**等待的类型**：

---

## **1. 线程的两种等待情况**

### **✅ （1）I/O 等待（阻塞等待，不占用 CPU）**

- **场景**：线程正在等待 **网络请求、数据库查询、磁盘读写** 等 I/O 操作完成。
- **状态**：线程进入 **阻塞（Blocked）或等待（Waiting）状态**，操作系统会让出 CPU 资源。
- **CPU 是否占用**：❌ **不占用 CPU**（线程不会执行，只等 I/O 完成）。
- **示例**：
    - 读取文件 `file.read()`
    - 数据库查询 `connection.execute()`
    - `Thread.sleep(1000)`（主动休眠 1 秒）

---

### **✅ （2）自旋等待（占用 CPU）**

- **场景**：线程在等待某个**变量改变**（如 CAS 自旋锁）。
- **状态**：线程不断**轮询检查变量**，不会主动释放 CPU 资源。
- **CPU 是否占用**：✅ **占用 CPU**（线程不断运行，消耗计算资源）。
- **示例**：
    - 自旋锁（SpinLock）：
        `while (!flag) {      // 自旋等待，不释放 CPU }`
    - `while(true)` 循环等待：
        `while (!taskCompleted) {     // 持续检查任务是否完成，占用 CPU }`
...
1. **I/O 操作（文件、网络、数据库）**
2. **`Thread.sleep(time)`（休眠）**
3. **`wait()` / `join()` / `park()`（进入等待状态）**
4. **线程阻塞（`synchronized` / `Lock` 等锁等待）**
...
1. **执行计算任务（CPU 密集型任务）**
2. **自旋锁（不停轮询变量）**
3. **忙等待（`while(true)` 死循环）**
4. **线程池的 `keepAliveTime = 0`，空闲线程不停检查任务队列**

---

## **4. 结论**

- **I/O 等待（网络、文件、数据库）时，线程不会占用 CPU**（系统会让出 CPU）。
- **计算任务、自旋等待时，线程会占用 CPU**（线程持续运行，导致 CPU 使用率升高）。
- **如果需要等待但不想占用 CPU，推荐使用 `Thread.sleep()` 或 `wait()` 机制**。

🚀 **推荐：在高并发系统中，尽量使用 `wait()` / `park()` 让线程挂起，而不是使用 `while(true)` 轮询，避免 CPU 资源浪费！🔥**